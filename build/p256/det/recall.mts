/**
 * Phase 256 prototype — RECALL against hand-enumerated ground truth.
 *
 * RESEARCH PROTOTYPE. Nothing here ships.
 *
 * Each scope below is a NAMED, BOUNDED region of one repository whose ground
 * truth the researcher enumerated by reading it on 2026-09-10. The enumeration
 * is re-derivable: every scope states the pattern it was counted with, and the
 * count is asserted here so a later reader running this script over the same
 * commits either reproduces it or is told the corpus moved.
 */
...
interface Scope {
  ...
  category: string;
  what: string;
  /** How the ground truth was counted, quoted so a reader can re-run it. */
  how: string;
  /** The count the researcher read on 2026-09-10. */
  expect: number;
  /** Ground-truth locations: `file:line` strings. */
  truth: (root: string) => string[];
  /** How a ground-truth entry is matched to a fact. */
  key: 'name' | 'line' | 'file';
  /** Which emitted facts count as an answer for this scope. */
  match: (f: Fact) => boolean;
  /** Only facts inside the scope are counted as extras. */
...
const SCOPES: Scope[] = [
  ...
  },
  {
    ...
    how: 'READ BY HAND, 2026-09-10: src/requests/adapters.py, sessions.py and api.py opened and every line that actually moves bytes or dispatches to the thing that does, written down. Five lines, listed here rather than derived, because no pattern distinguishes them from the two hundred lines that configure them.',
    expect: 5,
    key: 'line',
    truth: () => [
    ...
    match: (f) => f.category === 'effect' && f.kind === 'net',
...
console.log('RECALL against hand-enumerated ground truth\n');
...
for (const s of SCOPES) {
  ...
  const facts = (JSON.parse(readFileSync(join(factsDir, `${s.repo}.json`), 'utf8')) as { facts: Fact[] }).facts;
  ...
  console.log(
    [s.repo, s.category, s.what, truth.length, found, `${((100 * found) / Math.max(1, truth.length)).toFixed(1)}%`, extras, agrees].join(' | ')
  );
}
console.log('\n(“extras in scope” are facts of the matching kind inside the scope that name no ground-truth line; they are not necessarily wrong — a route rule may find a mount a line-based enumeration did not count.)');
