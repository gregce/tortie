#!/usr/bin/env node
/**
 * THE PHASE 250 ATTACK WIDENED, READ OFF THE RUNNING APP.

 * Written by the phase's VERIFIER and kept by the committer, because it is the
 * second instrument on the live surface that Tier 3 asks for: its own fixture,
 * its own shapes and its own assertions, independent of the builder's
 * `build/p247/path-open.mjs`.
 *
 ...
 *
 *   node build/harness-socket.mjs --fresh gmux-p250rv 'node build/p250/reverify-attack.mjs'
 */
...
// The scratch world.
// ---------------------------------------------------------------------------
mkdirSync(join(harnessDir, 'p250rv'), { recursive: true });
const root = realpathSync(join(harnessDir, 'p250rv'));
...
// RE-VERIFIER: a SIBLING of the project sharing its whole name as a prefix.
// `<root>/w` is the base; `<root>/w-evil` starts with it and is not inside it.
const sibling = `${project}-evil`;
rmSync(sibling, { recursive: true, force: true });
mkdirSync(join(sibling, 'docs'), { recursive: true });
writeFileSync(join(sibling, 'docs', 'sib.md'), '# the prefix that is not a parent\n');
// RE-VERIFIER: a symlinked leaf out of the base, pressed at the END of a row,
// which is the two lifts multiplied together.
symlinkSync(join(outside, 'stolen.md'), join(project, 'docs', 'endlink.md'));
...
const ARMS = [
  ...
  // ---- THE RE-VERIFIER'S OWN ARMS ----
  ['vP', '../w-evil/docs/sib.md end', '../w-evil/docs/sib.md', [], 0,
    'a SIBLING sharing the base name as a prefix — the separator clause'],
  // QUOTED, because an unquoted `~evil` is expanded by the shell that echoes
  // this row and the token never reaches the pane at all.
  ['vQ', "'~evil/docs/ok.md' end", '~evil/docs/ok.md', [], 0,
    'a tilde spelling that is not a home — never joined to a project'],
  ['vR', 'docs/endlink.md', 'docs/endlink.md', [], 0,
    'a symlinked leaf out of the base, at the END of a row — both lifts at once'],
  ['vS', 'docs/../docs/ok.md end', 'docs/../docs/ok.md', ['ok.md'], 0,
    'CONTROL: a relative climb that lands back INSIDE the base'],
  ['vT', './docs/ok.md end', './docs/ok.md', ['ok.md'], 0,
    'CONTROL: a dot-slash relative spelling']
];
...
/**
 * WHETHER THE TERMINAL ITSELF THINKS THE POINTER IS ON A LINK, POLLED (the
 * committer's round).
 *
 * This run grades by what OPENED, so a hover that had not resolved yet reads
 * exactly like a door that refused. A flat sleep is a bet on a round trip that
 * goes through IPC, `lstat`, `realpath` and `stat`; over three runs of the
 * builder's probe the same wait, polled and timed, came back **0 to 103 ms
 * over 18 waits, median 1 ms** — so the budget below is about forty times the
 * worst reading rather than a number chosen by feel. Polling can only turn a
 * false into a true, so every arm that must refuse now spends the whole budget
 * before it is allowed to say nothing was underlined.
 */
const ON_LINK = `document.querySelector('.xterm-cursor-pointer') !== null`;
const LINK_WAIT_MS = 4000;

async function waitOnLink(cdp) {
  const started = Date.now();
  for (;;) {
    if ((await cdpEval(cdp, ON_LINK, 10000)) === true) return true;
    if (Date.now() - started >= LINK_WAIT_MS) return false;
    await sleep(100);
  }
}
