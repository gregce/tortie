/**
 * Phase 234. Build and remove the scratch repository on the operator's Mac Pro.
 * A copy of .p224/far-fixture.mjs with this phase's own prefix, the same 26
 * file tree with three commits, a side branch and two untracked files, PLUS a
 * `docs/arch/` contract so the far side carries the thing this phase reads,
 * and one component whose anchor is hostile so independent method two has its
 * plant already on the far side.
 *
...
export const FAR_ROOT = `/Users/gdc/tortie-p234-scratch-${String(process.pid)}`;

function assertOurs(path) {
  if (path !== FAR_ROOT || !/^\/Users\/gdc\/tortie-p234-scratch-\d+$/.test(path)) {
  ...
  return [
    put(
      ...
      JSON.stringify(
        {
          version: 1,
          subject: 'The p234 fixture, a scratch repository on another machine',
...
function files() {
  ...
  out.push(put('README.md', '# p234 fixture\n\nA scratch repository made by Tortie phase 234.\nneedle-alpha appears in the source.\n'));
  ...
  out.push(put('package.json', '{"name":"p234-fixture","version":"1.0.0"}\n'));
...
cd ${root}
export GIT_AUTHOR_NAME='Tortie P234' GIT_AUTHOR_EMAIL='p234@example.invalid'
export GIT_COMMITTER_NAME='Tortie P234' GIT_COMMITTER_EMAIL='p234@example.invalid'
${files()}git init -q -b main
git config --local commit.gpgsign false
git config --local user.name 'Tortie P234'
git config --local user.email 'p234@example.invalid'
git add -A
git commit -q -m 'p234 base: the tree and its contract'
${put('lib/helper5.ts', 'export const helper5 = 5; // needle-beta\n')}git add -A
git commit -q -m 'p234 second: another helper'
${put('docs/changelog.md', '# changelog\n\n- first\n')}git add -A
git commit -q -m 'p234 third: a changelog'
git checkout -q -b p234-side
${put('docs/changelog.md', '# changelog\n\n- first\n- side branch line\n')}git commit -q -am 'p234 fourth: on the side branch'
git checkout -q main
