...
- Saving a file that changed on disk while you were typing no longer writes over it in silence. Tortie says so and offers three answers: Compare, which opens the two versions side by side, Cancel, or Overwrite, which is never the default. Overwriting is checked again as you press it, so a write that arrives while the question is on screen is caught as well. A file outside your open projects, and a file that is a symbolic link, still save the way they always have and are not checked ([`2a219236`](https://github.com/gregce/tortie/commit/2a219236)), ([`06e5e909`](https://github.com/gregce/tortie/commit/06e5e909)), ([`3b738bfa`](https://github.com/gregce/tortie/commit/3b738bfa))
- A save that cannot happen now says why, whether the file is read-only, gone, too large, outside your projects or not text Tortie can rewrite without damaging it. It used to say the same sentence twice and name no cause ([`3b738bfa`](https://github.com/gregce/tortie/commit/3b738bfa))
