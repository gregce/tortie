...
- Typing in one file in the Redline view and then clicking another tab no longer puts the first file's text into the second. That tab used to turn unsaved with its whole file shown as deleted, and saving it wrote the first file's text over it with nothing asked, which auto save did with no key pressed
