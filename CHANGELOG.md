...
- The Redline tab for a Markdown or text file now marks every change since the last commit, or since you opened the file when it is not in git, and it redraws by itself when an agent writes to the file. The marking is remembered between sessions and lasts until the file's committed version changes, and it waits while you have unsaved edits ([`8e036fc`](https://github.com/gregce/tortie/commit/8e036fc)), ([`f41a5e3`](https://github.com/gregce/tortie/commit/f41a5e3)), ([`94e21c6`](https://github.com/gregce/tortie/commit/94e21c6))
...
- What you accept in a redline now outlasts the tab. Close the file, reload the window, or quit and come back, and the marking is still narrowed to what changed since you accepted, with the day it was taken written beside it — until the file's committed version changes, or a week after you accepted, when the redline goes back to marking every change since the commit ([`4140c73`](https://github.com/gregce/tortie/commit/4140c73)), ([`3118cd3`](https://github.com/gregce/tortie/commit/3118cd3)), ([`b5ffe89`](https://github.com/gregce/tortie/commit/b5ffe89)), ([`c00be09`](https://github.com/gregce/tortie/commit/c00be09))
...
- Saving a file that changed on disk while you were typing no longer writes over it in silence. Tortie says so and offers three answers: Compare, which opens the two versions side by side, Cancel, or Overwrite, which is never the default. Overwriting is checked again as you press it, so a write that arrives while the question is on screen is caught as well ([`2a219236`](https://github.com/gregce/tortie/commit/2a219236)), ([`06e5e909`](https://github.com/gregce/tortie/commit/06e5e909)), ([`3b738bfa`](https://github.com/gregce/tortie/commit/3b738bfa))
- A file that is a symbolic link, and a file outside your open projects such as a global CLAUDE.md, are asked about too. They are checked a moment before they are written rather than as they are written, so a write that lands in that moment is still lost ([`f89284f2`](https://github.com/gregce/tortie/commit/f89284f2))
- A save that cannot happen now says why, whether the file is read-only, gone, too large, in a project you have closed or not text Tortie can rewrite without damaging it. It used to say the same sentence twice and name no cause ([`3b738bfa`](https://github.com/gregce/tortie/commit/3b738bfa)), ([`f89284f2`](https://github.com/gregce/tortie/commit/f89284f2))
- A file that is not plain UTF-8 text is no longer told that something wrote to it when nothing did. It says the file is not text Tortie can rewrite, and it leaves every byte where it was ([`f89284f2`](https://github.com/gregce/tortie/commit/f89284f2))
