...
- A sign in of any size is kept, so a Codex account you sign out of can be offered back, which never once worked. Tortie now keeps its copies of your sign ins in a sealed file of its own instead of your keychain and moves the copies already there over on their own; a Claude sign in that has grown larger than Tortie can write into the agent's keychain entry in one line still cannot be put back ([`09c2ff56`](https://github.com/gregce/tortie/commit/09c2ff56))
