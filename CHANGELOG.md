...
- Choosing one of your other sign ins no longer writes over the sign in you were already using, and a login whose sign in has grown too large for the agent's own keychain entry can be chosen again. When that entry cannot take the sign in, Tortie says so and the session already running keeps its current sign in until you press Restart now
- A sign in of any size is kept, so a Codex account you sign out of can be offered back, which never once worked. Tortie now keeps its copies of your sign ins in a sealed file of its own instead of your keychain and moves the copies already there over on their own; a Claude sign in that has grown larger than Tortie can write into the agent's keychain entry in one line still cannot be put back
