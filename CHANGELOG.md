...
This release is about the redline you can rewind, accept and now type in. Every edit to a prose file shows up as a marking against what the file was, whether git has noticed it or not, and a change you point at either goes back with one key or stops being marked with another, without the file being touched. The editor answers a right click now too, saving a file somebody else has written to asks you first, and a file an agent names in a session can be opened by pointing at it.

### Added

- A file an agent names in a session is now a link. Point at it and it underlines; click it and it opens where it belongs — prose and code in the editor, a picture in the image view, and a PDF, which Tortie cannot draw, in whatever your Mac opens PDFs with.
- Only files that are really there are offered, and a path that runs off the right edge of the pane is left alone, because where it ends is not known. A session running on another machine offers no file links at all, since a path there reads exactly like a path here and is a different file.
- Anything that could run is never offered: a file with the executable bit set, an application, or anything wearing a picture’s name that is really a folder. Neither is a file whose name says it holds a secret, which now covers auth.json, .npmrc, .git-credentials and their family as well as the dotenv and key files it already covered.
