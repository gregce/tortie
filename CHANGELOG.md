...
- A path printed at the end of a line is a link too. It used to be left alone along with paths that run off the right edge of the pane, and the end of a line is where an agent almost always writes one; a path that really does run off the edge is still left alone ([`3649185`](https://github.com/gregce/tortie/commit/3649185))
- So is a path written relative to the project, which is most of what an agent prints. It opens the file under the project that session is in, and a name that resolves outside that project is left alone; a relative name is never handed to your Mac, so a PDF named that way opens nothing, and a name printed by an agent working in another copy of the same project can open that file in the wrong copy ([`5c21595`](https://github.com/gregce/tortie/commit/5c21595))
