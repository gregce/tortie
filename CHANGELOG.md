...
- Tortie has an iPhone app that lists every session with the ones waiting on you first, shows how each is going and scrolls back through its whole conversation, over your own tailnet with nothing else to install on the phone: switch it on in Settings then Phone, paste a one-time Tailscale key and scan the code. It only reads, so you still answer and end sessions on the Mac, which must be awake with Tortie open and the Tailscale app running, and the app is on TestFlight rather than the App Store ([`38346773`](https://github.com/gregce/tortie/commit/38346773)), ([`8c7f0b2f`](https://github.com/gregce/tortie/commit/8c7f0b2f)), ([`d19a906a`](https://github.com/gregce/tortie/commit/d19a906a)), ([`eaef6ee4`](https://github.com/gregce/tortie/commit/eaef6ee4)), ([`9d1a3001`](https://github.com/gregce/tortie/commit/9d1a3001))
...
- Choosing one of your other sign ins no longer writes over the sign in you were already using, and a login whose sign in has grown too large for the agent's own keychain entry can be chosen again. When that entry cannot take the sign in, Tortie says so and the session already running keeps its current sign in until you press Restart now ([`09c2ff56`](https://github.com/gregce/tortie/commit/09c2ff56))
- A sign in of any size is kept, so a Codex account you sign out of can be offered back, which never once worked. Tortie now keeps its copies of your sign ins in a sealed file of its own instead of your keychain and moves the copies already there over on their own; a Claude sign in that has grown larger than Tortie can write into the agent's keychain entry in one line still cannot be put back, and going back to an earlier version of Tortie hides the sign ins it is holding until you come forward again, so do not sign into a different account in your own location while you are back there ([`09c2ff56`](https://github.com/gregce/tortie/commit/09c2ff56))
...
- Manage Sessions no longer searches every one of your Claude Code project folders each time it asks about a session you have not sent a message to yet: within half a minute the answer is remembered, so filling the sheet again over such sessions takes about a hundredth of what the first fill took. A very large conversation still holds the window on the sheet's first open exactly as before ([`7b64efa5`](https://github.com/gregce/tortie/commit/7b64efa5))
...
- Typing in one file in the Redline view and then clicking another tab no longer puts the first file's text into the second. That tab used to turn unsaved with its whole file shown as deleted, and saving it wrote the first file's text over it with nothing asked, which auto save did with no key pressed ([`0f0cdc6f`](https://github.com/gregce/tortie/commit/0f0cdc6f))
