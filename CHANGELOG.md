...
- Manage Sessions no longer searches every one of your Claude Code project folders each time it asks about a session you have not sent a message to yet: within half a minute the answer is remembered, so filling the sheet again over such sessions takes about a hundredth of what the first fill took. A very large conversation still holds the window on the sheet's first open exactly as before.
