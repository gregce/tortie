...
**The count.** Of the fifty one phases here, **35 had a verifier finding that was fixed**, either inside the phase or in a later phase named in the section. **16 had no verifier finding that led to a fix**, and for each of those the last section says what the verifier did instead, so "nothing found" can be told apart from "nothing looked for". A final section lists the things verifiers found that are still not fixed, so nothing is quietly lost.

**The fifty, in the order they landed.** 168, 169, 169.1, 169.3, 170, 171, 174, 172, 176, 177, 173, 183, 178, 179, 180, 184, 175, 175.1, 181, 174.1, 181.1, 181.2, 185, 186, 187, 188, 189, 182, 174.2, 192, 193, 188.1, 191, 194, 200, 202, 190, 201, 195, 196, 198, 197, 199, 203, 204, 206, 205, 208, 207, 209 and, added at the end, 210. Phase 210 landed at `bae02df` while this document was being written, so it is added as the last section and the title keeps its fifty, as the phase entry said it would. Phase 211 is still in flight and is not here.
...
A few words used below: a **gate** is a check that runs at commit or build time and fails loudly. A **probe** is a script that launches the real app and reads it. A **fixture** is a made up input for a test. The **keychain** is where macOS keeps passwords. A **symbolic link** is a file that points somewhere else. The **harness** is the test rig that launches the app and drives it the way a person would, so a probe can read what it did. The **charter** is the phase's brief in the backlog, the words the phase was asked to do. An **ablation** takes one line out of a fix and re-runs the check, to prove the check goes red without that line.
...
| 174.2 | The font field sits level with its dropdown | The level half shipped with no guard that could go red; the verifier ruled where one belongs | Re-derivation, attack | `30b6d1e` (Phase 197) |
...
| 210 | The frame goes darker and lighter | A floor's stated reason was a measurement that reads 5, not 2, in four places and then a fifth; a walk maximum was one hue's reading | Re-derivation | `c14dd2a`, `d0ff340`, `bae02df` |
...
**What went wrong.** The feature did not answer its own switch. Turning a meter on drew nothing for up to fifteen minutes, and turning it off left the numbers on screen. The store started polling once behind a flag, only re-asked after fifteen minutes, and nothing listened for settings changes. A person flipping the switch would conclude it was broken. Three notes: the strip drew any percentage it was given, including 500 and a value that is not a number at all; nothing bounded a reset date, so one hover card read "Resets in 11574053377d"; and the Claude endpoint answers a request with no credential with the code for too many requests, 429, rather than the code for not signed in, 401.
...
**How it was fixed and why.** `5f58b2a` caps the wait at six seconds a mode. `662457b` names the toggle and spells color as the app does. `343cb80` shows the control row only when the diff surface itself says it has changes to mark, the same condition the surface uses, and guards the flash with a reading proved red under the old gate. The probe is registered as `npm run probe:p185`, and the spec records where a diff preference lives. `eb61f53` corrects the recorded size of the code the app loads at start, called the eager set. The two re-verify blockers landed in Phase 197 at `5e45f37`. Version 0.96.0 at `c9dbf5a`.
...
## Phase 174.2: the custom font field sits level with its dropdown

**What the phase was.** The operator sent a screenshot: on Settings then Appearance, the Custom font text field sat 9 pixels higher than the dropdown beside it. The cause was Phase 174.1's reserved line for the "not installed" note. It sat in the same column as the field and made the row centre a taller box. The phase moves the note out of the column's flow and keeps 174.1's promise that the field never moves. Landed at `7917485`.

**What went wrong.** Nothing the verifier called a defect; it approved. But the level half shipped with no guard that could go red, and the builder had left the ruling on that to the verifier. The only byte level test on offer would have pinned the mechanism, an absolute placement, rather than the promise, and would have blocked a later fix that reaches level by another route. Two advisories. One, the comment above the rule forbids three undos but does not record that the row must stay tall enough to hold the note, a risk measured at 43 pixels of slack today. Two, the guard belongs as an arm on the existing font field probe, which already drives the real field and reads rectangles.

**How the verifier found it.** Re-derivation three ways the builder did not use, then attack. It worked the offset out by hand from the box model with no rectangle read. It stepped Chromium's own hit test down a column a quarter of a pixel at a time. And it wrote its own PNG reader and found each control's drawn edge in the photograph's bytes. All three read minus 9 pixels at the parent `bd79dba` and 0 at the tip. It attacked with a note sentence four times longer at runtime and 32 frames per commit, and re-ran the Phase 174.1 probe that the commit battery never runs.

**How it was fixed and why.** The probe arm landed in Phase 197 as item 16 at `30b6d1e`, whose body names Phase 174.2. The probe now reads the dropdown's rectangle beside the field's in every frame of its typing sequence and requires the centres and the top edges to agree. It names no CSS property, so any stylesheet that lands the field level passes and any that does not fails. Proved red at the parent stylesheet, 7 of 7 frames off by 9 pixels, and green at the tip. The first advisory, one comment sentence on the row's height, was not applied; the comment at `7917485` still records the three undos and not the slack. The guard is `npm run probe:p1741`, which is not in the commit battery.
...
**What went wrong.** Item 24 added a new hairline colour for a border drawn on the selected fill, and its commit body claimed exactly one place needed it. There were two. The Aim button in the Architecture pane also drew the old hairline on the same fill when hovered, at a contrast of 1.013 to 1, which is the very defect the token exists to fix. Second, one commit in the middle of the round failed the typecheck gate on its own, because its fix sat in a later commit. The round's reason for one commit per nit is that a bisect, which walks the commits one at a time to find the first bad one, can name the one that broke something, and a red commit in the middle defeats that.

**How the verifier found it.** Re-derivation, twice. It wrote its own CSS scanner that joined every rule painting the active fill to the border its base rule wears, over 2,223 rules in 65 files, and found two consumers. It decoded the harness photograph with its own reader and read the edge pixels. For the second finding it ran the import boundary gate over each commit separately, which the builder had only run at the tip. It also attacked the font sanitizer with invisible characters that ride through it whole. The first verifier counted 21 of 22; the second counted 19 of 24, and nineteen is the number the landed record carries.

**How it was fixed and why.** One line so the Aim button's hover takes the new hairline, the commit body corrected to two consumers, and the probe reads that rule from the live stylesheet, `1371500`. The red commit was squashed into the one it fixed, `c366086`, so every commit in the round passes alone. The second verifier found three comment lines still naming the wrong kind of fill, amended inside the same commit. The nineteen invisible characters were recorded rather than fixed here, outside the item's charter, and Phase 206 fixed them at `b5c2523`.
...
## Phase 210: the ramp's own lightness

**What the phase was.** After Phase 207's hue slider, the operator said he expected to pick shades, from black through grey to white, and that he did not understand the degree setting. The phase replaced the degree slider with a row of eight named starting colours, each drawn as the frame it would produce, and added two sliders: Shade, for how dark or light the whole app sits, and Depth, for how far its panels and hairlines stand apart. A setting the contrast floors cannot keep is refused at the control with a short line saying which other slider rescues it. Near black is reachable; black, grey and white are not, and the phase says why. Landed at `bae02df`, version 0.99.0 with no bump, running log line at `37616e1`.

**What went wrong.** A stated measurement that did not reproduce. The phase set a floor: two neighbouring greys in the ramp must be at least two eight bit levels apart, so a rung of the ramp stays a rung. Its reason, written in four places, was that two is what the shipped ramp holds at its tightest. It is not. By the rule's own six pairs the shipped ramp holds five at its tightest, and 5, 9, 9, 8, 10 and 11 at the shipped frame. Two was what the offered region bottoms out at, which is circular, because the region is defined by that floor. The phase had just refused a different floor for being a number nobody can re-derive, and then wrote one of its own. Smaller: the lightest canvas the ramp reaches was quoted as Y 0.0138, which is one hue's reading and not the maximum over every hue, which is 0.0147; and a comment in the slider range code stated a case that cannot happen. After the fix round the re-verifier found a fifth place still carrying the false clause and the old number: the conventions file, the one a later agent reads before touching the ramp. Also found and recorded, not fixed: the idle and exited status dots fall under the 3 to 1 contrast floor for non-text over part of the offered region.

**How the verifier found it.** Re-derivation by another route, plus one thing nobody else did. It wrote its own colour arithmetic sharing nothing with the tree, its own OKLab conversion, its own gamut clamp and its own contrast formula, and ran it beside the shipping code over 9,408 derivations and 366,912 colours with zero byte mismatches. That same arithmetic, asked what the shipped ramp holds at its tightest, answered five and not two. It re-derived the whole table of offered frames from scratch and matched it cell for cell. And it attached the node inspector to the main process and read the window's own background colour out of main itself, so the fill was read rather than computed. The re-verifier repeated the re-derivation with its own arithmetic over 64,017 override maps and swept every file for the old sentence, which is how the fifth place surfaced.

**How it was fixed and why.** The floor did not move; its reason did. `c14dd2a` rewrites the reason in all four places as a physical one: one eight bit level is the least difference the format can express, so a pair one level apart cannot be told from two colours that meant to be the same, and two is the smallest step above that. The fix round did not take the verifier's suggested wording, because "one level is one colour" is not quite true either. The same commit quotes the walk's true maximum, Y 0.0147 at hue 186, beside the shipped hue's 0.0138, and the gate now measures that number where it prints it. `d0ff340` corrects a fifth place for that number that the verifier had not named, the app run's own header. `b4776a7` records the fix round and queues the status dot question for the operator, with the fix round's own count of 7 of 35 offered cells under the floor where the verifier had said 11 of 40. The re-verifier then came back needs_work on the conventions file, and the committer re-derived the six pairs from the hex with no library at all, 5, 9, 9, 8, 10 and 11, before correcting both clauses there in `bae02df`. No code moved, no gate moved, and the region did not move by one cell. The status dots are not fixed: pinning them would refuse the shipped default, so the choice is the operator's.
...
- **Phase 169.1, the Oh My Pi mark goes monochrome** (`880e47d`). No verifier. The operator's own commit nine minutes after Phase 169 landed. A follow up docs commit `9e5eedd` fixed the icon header.
...
- **Phase 195, the chrome stops contradicting itself** (`f29861f`). Approved. The verifier built the parent and HEAD, drove sixteen photograph probes at each, and decoded every picture with its own decoder: zero pixels moved on eleven pictures, the dot alone changed on three, and every other difference was cropped, viewed and named as run noise that was present at the parent too. Two things noted for the record, neither a defect under the phase's rule.
