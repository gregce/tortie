...
| 3 | The word the copy may not use | **Not "remote".** The copy names the machine's label, and it lives in `src/renderer/machines/presentation.ts` | `src/renderer/scm` holds 285 non-test occurrences of "remote" and every one of them means a git remote. Adding a second meaning to that word inside that view is the one copy change that cannot be undone by editing a string |
...
Every new string belongs in `src/renderer/machines/presentation.ts`, beside `reviewListTitle`,
...
| The badge and the vocabulary | `src/renderer/app/MachineBadge.tsx`, `src/renderer/machines/presentation.ts`, `src/shared/machines.ts` | `MachineBadge`, `reviewListTitle`, `reviewItemLabel`, `createDirLabel`, `MACHINE_COLORS` |
