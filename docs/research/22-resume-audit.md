...
| 8 | **All agents** — behaviour when two panes in one cwd start within the same second | only pi's collision was proven (and it is a *partial-id* collision, a different mechanism) | matters only for Tier 2/3; muse (`tmux_pane`) and qwen (`pid`) are immune by construction, deepseek and antigravity are not |
