...
Written 2026-09-10, at `2e1b4799`, in the worktree `/private/tmp/wt-p256`. This is the DESIGN step of
...
`build/p256/det/measurements/`, `build/p256/out-explorers.json`, `build/p256/pane-readings.json` and
`build/p256/overlay-readings.json`. The prototype is `build/p256/det/`, the hand-written semantic
pass and its checker `build/p256/semantic/`, the mock `build/p256/mock/`.
...
fact base. **The checker's value is not that it refuses lies. It is that it says, per sentence, which
ones are standing on something.** A surface built this way must DRAW that difference rather than hide
it, because hiding it would claim 100% evidence for 58.5%.
...
strictly better, and nothing is thrown away if the model half is never used.
...
Four of the thirteen shipped grammars — java, php, c-sharp, kotlin — were exercised by no repository
in the corpus, so every rule naming only those is **written and unmeasured**, and this document does
not claim otherwise.
...
gate 60% (69% excluding vendored) — **79% overall, 84% excluding vendored**. Per repository: tortie
95%, fastapi-app 95%, requests 93%, stoa 86%, mastodon 81%, ripgrep 70%, babel 67%, gotify 66%,
alamofire 61%.
...
guarded write is exactly the shape that hides a surface from a name rule. It costs about a doubling of
the read.
...
(`arch_import`), the anchors (`docs/arch/`), and the new entrypoint and test facts, whose measured
precision is 91% and 96%. `reached` is a walk over a table that already exists.
...
- **backed** — a fact of the wanted shape within the claim's span;
- **resolves** — a real tracked line, nothing of the wanted shape near it;
- **broken** — the row is refused whole at validation and never reaches the face.

Measured on a careful hand-written pass: 41/41 resolve, **24/41 backed**. A surface that drew all
forty-one the same way would be claiming 100% evidence for 58.5%. So the chip is on the face, the
component header carries `n of m backed`, and the provenance line at the top of the view carries the
whole-repository ratio. **This is the single most important consequence of §6.4 for the design.**
...
Three reasons it is not in `docs/arch/`, in order of weight. **(a)** It would need about fifteen new
...
warning against. **(c)** Prose arriving from a stranger's push would be drawn on the person's face —
which the key set's letter permits, since prose names no command, and which its REASON does not.

**One exception is real and free.** `ArchFlow` is already in the pinned key set and is read by nothing
today, and it is field for field a journey. **A journey may live in `docs/arch/` with no pin moved and
no key added.** That is worth taking: a journey is the one semantic artifact a person might genuinely
want to review in a diff and carry to a clone.
...
**Refusal 8 is untouched: no model turn starts on a file change alone.** Staleness is DRAWN, and the
...
Nothing in this design lets a `git pull` start a process.
...
- **a process partition rather than a directory one** — what this repository BUILDS and STARTS, from
  manifests, Dockerfile and compose services, CI jobs, workspace members and library targets;
  boundary facts measured at **100% precision on 45 judged**. It is labelled for what it is: not
  "where code runs", which is a model claim, but "what this repository builds and starts";
...
recomputed by the builder from the deterministic prototype's fact file over this repository at
`2e1b4799`, by the same three-line rule `build/p256/semantic/check.mts` applies; an amber `prose` chip
means the link resolves to a real tracked line and the deterministic half found nothing of the wanted
shape near it. The counts in the masthead are read out of that fact file. **What is the researcher's
and is not measured** — and the footer says so too — is which region a part sits in, its three-to-five
word label, and the name on each transport.

**Measured off its own DOM**, in the same scratch Electron at 1440×900 that read the three explorers:
3 tabs, 4 regions, 3 named transports, 9 nodes, **278 words above the fold** and 521 in the body,
...
argv. No fact written into `docs/arch/`.
...
between a deterministic and a semantic mode — there is one way in (Phase 158).
...
scan proving **no spawn is reachable from a watcher event** (refusal 8, asked structurally rather than
by comment); the prompt cap asserted at its measured byte size; and the second recipe's measurement
recorded the way the first one was, or it stays `not-measured` and disabled.
...
1. **Four grammars are unmeasured** — java, php, c-sharp and kotlin were exercised by no repository in
   the corpus, so every rule naming only those is written and untested.
2. **No agent was measured writing a semantic pass.** The pass is the researcher's, by the charter's
   own instruction. The 58.5% is therefore a measurement of what a CAREFUL writer's citations can be
   bound to, and a model's number is unknown and could be worse.
...
6. **The regions in the mock are the researcher's**, and whether a model can produce a good region
   partition on an unfamiliar repository is exactly the thing nobody has measured.
