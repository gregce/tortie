...
| orchestrator | FIXED, recorded so the reason survives. The runner template said `REPO: /Users/gdc/gmux` and never named the phase worktree, and it told the committer to run `git pull --rebase origin main` with no directory. That is why a committer used the operator's checkout. The template now requires a `worktree` argument and throws without one, names it in the first line every agent reads, forbids `/Users/gdc/gmux` by name, and makes the committer's first step a `git rev-parse --show-toplevel` check that it is not there |
| orchestrator | `node --check` returns exit 0 on a workflow runner script that does not actually parse, because the script is a module body with top-level await rather than a plain script. A broken escape survived that check and was caught only by compiling the source inside an async wrapper and comparing against a known-good control. This is the same lesson as the matrix runs: an exit code is not a result |
...
| 77 | The crash item 3 guards against was never reproduced. The `void` shape also exited 0 in the warm probe, so awaiting the two disposals is a precaution against a class of abort rather than a fix for a measured one |
