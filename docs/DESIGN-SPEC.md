...
- Container: fixed bottom-right, 16px inset, `--z-toast`, stack gap 8px, max 3 (older collapse to a "+n" line). One exception, and it is the only one: while the session manager is open the same stack DOCKS inside the sheet, above its footer, as S15's toast strip — same element, same order, same rules, without the fixed placement. A fixed toast at `--z-toast` sits over `--z-modal` and took the clicks meant for the sheet's own verbs on its lowest rows.
...
- A scrolled pane keeps its place, across every change of size and not only zoom. Since Phase 292 nothing in the app re-scrolls a parked pane after a resize: tmux keeps the line its copy cursor is on across a reflow, and main puts that cursor on the reader's top row after every scroll that parks (`cursorToTopRow`, src/main/tmux/scroll.ts). The hold that re-sent the old position once the resize landed (Phase 12.11, drift 10 lines → 0 with main's correction still running) is deleted, because the position counts rows and over soft-wrapped lines the same number is a different place: it threw a reader 127 lines on every change of width. While scrolled back the terminal's cursor therefore sits at the start of the top row. A pane's own answers to its terminal (the focus report, the colour reports, the device-attribute answers) are forwarded without leaving scrollback, through the one question in src/renderer/terminal/keys/pane-report.ts; only what a person types or pastes takes a reader back to live.
...
## S15 — Session manager (Session → Manage Sessions…) — Phase 293, its rows on the app's own scale in Phase 298

One sheet over every session Tortie manages, across every project and machine, whether or not the session's project has an open tab. Two tabs: **Managed**, and **Past Sessions**, which Session → Past Sessions… opens directly. Both menu rows work with no project open and neither carries an accelerator, for the reason End Session… carries none. The sheet replaces the Phase 29 Past Sessions modal; it is not kept beside it. Copy for the five states that replace the grid is DESIGN.md §6.16 to §6.20, verbatim.

```
┌ .modal.session-sheet  w:min(1180px, 100vw - 48px) · bg --bg-surface · 1px --border · r:--r-lg · --shadow-3 ┐
│ Sessions    ▣ Managed 12   ↺ Past Sessions 4                                  ⟳  ×    │ title bar  h:52
│ [ Search sessions or projects      ] [All projects ▾] [All project tabs ▾] [All states ▾] │ toolbar    h:47
│ ☐   Session          State          Created       Messages     Last message           │ sticky headings, --bg-raised
│ ▭ tortie  3  ⟦Open tab⟧                                                 ~/src/tortie  │ group header (scrolls)
│ ☐  ◉ claude-auth     ● Working      Today          41        2m ago     [End session…] ⋯ │ row
│ ☑  ◉ codex-fix       ○ Ended        Sep 16         12+       2d 1h ago  [Restore]      ⋯ │ checked: --bg-active
│    ┌ inline panel: --bg-raised · 1px --border-strong · r:--r-md ─────────────────────┐ │ one at a time, under its row
│    └────────────────────────────────────────────────────────── [Cancel] [End session] ┘ │
│ 12 managed sessions                                                                   │ footer, --text-2xs --text-muted
└───────────────────────────────────────────────────────────────────────────────────────┘
```

**Rules that bind every line of its CSS and copy.** Tokens only, and no token was added for it; the geometry numbers are custom properties on `.session-sheet` — the frame's four (52, 47, 46, 1180) and, since Phase 298, the row's own `--sm-row-h: calc(28px + 2 * var(--space-3))` and the sticky heading's `--sm-head-h`, each written as the sum that decides it rather than as its total, plus `--sm-name-min`, which is MEASURED by `probe:p293` arm 11 and never chosen. `--text-muted` only on `--bg-surface`; on `--bg-raised` and `--bg-active` grounds text is `--text-secondary` (S12 item 4). A hairline drawn ON `--bg-active` is `--border-active`; between regions it is `--border`. Class prefix `sm-`; the root is `.modal.session-sheet`, and an override of a shared modal class carries that second class so it wins on specificity and never on bundle order. No multiplexer vocabulary (S12 item 6). The resting face carries labels; explanation lives in a heading's hover title or inside the Details expansion. One row scale, one type scale, one icon scale: every row in the sheet is `--sm-row-h`, every size in it is a size the app already draws, and a row under the pointer eases the way the session rail and the SCM list do. Reduced motion: the sheet uses `.modal`'s own entrance, and the fills it eases are written as `var(--dur-fast) var(--ease-out)`, which `tokens.css`'s app-wide `prefers-reduced-motion` block already refuses — so the sheet carries no reduced-motion block of its own.

**Geometry and the token map.**

| Element | Value | Token or reason |
|---|---|---|
| Scrim | the shipped `.modal-scrim` plus `.session-sheet-scrim`, whose `::before` is `flex: 0 0 var(--space-8)` | `--bg-scrim`, `--z-modal`; the 24px replaces the modal family's 20vh headroom, because the sheet wants the height |
| Sheet | `.modal.session-sheet`: `width: min(1180px, calc(100vw - 48px))`, `flex: 1 1 auto`, `min-height: 0`, `max-height: none`, `padding: 0`, `overflow: hidden`, a flex column | ground `--bg-surface`, `1px solid var(--border)`, `--shadow-3`, all inherited from `.modal` |
| Corner | `var(--r-lg)`, 10px | `--r-frame` has exactly one user, the work area's outline, and `--r-lg` is the modal radius |
| Title bar | `height: 52px; min-height: 52px; padding: 0 var(--space-7); gap: var(--space-8)`; bottom rule `1px solid var(--border)` | `--sm-title-h` |
| Title | `Sessions`, `--text-lg` / `--lh-lg` at `--weight-semibold` | `.modal-title` and `.empty-title` are the app's title step, and a 28px line box clears the 52px bar with 12px to spare |
| Tabs | `Managed` (codicon `terminal`) and `Past Sessions` (codicon `history`), label `--text-base` as `.ptab` and `.stab` draw it, each with a count chip that is the shipped `.chip-sm` primitive (16px tall, `--text-2xs`, `padding: 0 var(--space-2)`, `min-width: 16px` from `.ab-badge`, ground `--bg-raised`). Selected: `--text-primary` and a 2px `--accent` underline at `bottom: -1px`, inset by `var(--space-2)` with `border-radius: var(--r-bar) var(--r-bar) 0 0` | the counts are whole-tab totals and filters never move them. The two tab marks are named here, so a round tidying icon sizes keeps them; the underline matches `settings.css`, the app's only other tab underline |
| Title bar, right | refresh then close, icon buttons 28 by 28, `--r-sm`, `gap: var(--space-4)`, pushed right with `margin-left: auto` | |
| Toolbar | `height: 47px; box-sizing: border-box; padding: 0 var(--space-7); gap: var(--space-4); align-items: center; flex-wrap: nowrap`; bottom rule `1px solid var(--border)` | `--sm-toolbar-h`, stated outright in BOTH modes: `--field-h` is 28px and `.btn-sm` is 24px, so the two modes would not come out equal by themselves |
| Toolbar, filters mode | search `flex: 1; min-width: 180px`; project select 170px; tab select 136px; state select 114px (Managed only). Selects `--field-h`, `--text-sm` / `--lh-sm` | the four controls combine with AND. `--text-sm` is `--view-field-text`, the step every field in the app reads at minimum |
| Toolbar, selection mode | the SAME element, `data-mode="selection"`, ground `--accent-wash`: clear-selection icon button, `<n> selected`, the running and skipped counts, a spacer, `End selected sessions…` with codicon `debug-stop` | the same 47px, so selecting costs no height |
| Grid scroller | `.sm-scroll { flex: 1; min-height: 0; overflow: auto }`. The batch panel and the grid both live inside it | the batch panel draws above whatever state replaces the grid |
| Grid | a real `<table class="sm-grid">`, one `<tbody>` per group: `border-collapse: collapse; width: 100%; table-layout: auto; font-variant-numeric: tabular-nums`. It states NO `font-size`, so it inherits `body`'s `--text-base` / `--lh-base` | one name size: the grid's own `--text-sm` made `.sm-name` draw 12px here and 13px in the Past list, for the same name |
| Row | `tr.sm-row { height: var(--sm-row-h) }`, 40px, and its pitch is **41** | row height = tallest child + 2 × `--space-n`, and the tallest child here is `.btn`'s own 28px (`globals.css`, a literal there too). 32px rows would have RENDERED at 40 and 44 with a declared 32 no browser draws, and 28px rows put the 2px outer focus ring wholly outside the row, where `.sm-scroll`'s unreachable block-start overflow cuts it and the sticky `thead` paints over it. `border-collapse: collapse` puts the hairline BETWEEN two rows, which is the one pixel between 40 and 41 |
| Sticky headings | `thead th`: `position: sticky; top: 0; z-index: 1; background: var(--bg-raised); padding: var(--space-3) var(--space-5); font-size: var(--text-xs); line-height: var(--lh-xs); font-weight: var(--weight-medium); text-transform: uppercase; letter-spacing: var(--track-caps); color: var(--text-secondary); white-space: nowrap` — 28px. The rule under them is `box-shadow: inset 0 -1px 0 var(--border-strong)` | a border on a sticky `th` under `border-collapse: collapse` drops out in Chromium. The app's heading idiom is unanimous across 26 rules and this is it, with ONE deliberate deviation: the colour stays `--text-secondary` and never the idiom's `--text-muted`, because the heading's ground is `--bg-raised`, where `--text-muted` measures 4.15 and is under the floor |
| Data cell | `padding: var(--space-3) var(--space-5); border-bottom: 1px solid var(--border); vertical-align: middle; white-space: nowrap`. A cell's second line is `.sm-cell-small`: INLINE, `--text-xs` / `--lh-xs`, `margin-left: var(--space-3)`, `--text-secondary` | row hover `--bg-raised`, `transition: background var(--dur-fast) var(--ease-out)`. Every secondary field is inline and one step, because `.scm-row-dir`, `.qo-project`, `.set-row-caption` and `.rail-card-detail` all are; the 10px ratio line-height it replaces was the only one in either tree, and `--text-2xs`'s own token says never body text |
| Checked row | ground `--bg-active`, hairline `--border-active` | |
| Select column | `width: 46px; padding: 0 0 0 var(--space-4)`. Hit target 32 by 32, input 14 by 14, `accent-color: var(--accent)` | |
| Session column | `min-width: 155px`; the name `flex: 0 1 auto`, `min-width: var(--sm-name-min)`, `max-width: 175px`, ellipsis | agent mark at `<AgentIcon>`'s own 16px default, with the call site passing no size at all so it cannot drift again; then the name at `--text-base` / `--lh-base` medium, the machine badge for another machine only, and the agent inline beside it rather than stacked under it. `--sm-name-min` is measured the way `.ptab-name`'s 46px was, never chosen, and the number and its date live in the CSS comment |
| Actions column | `min-width: var(--sm-actions-min); padding-right: var(--space-7)`; `.sm-row-actions { display: flex; justify-content: flex-end; gap: var(--space-3) }` | ONE visible button, then the ellipsis. The floor is re-derived from what the cell HOLDS — the button, one `--space-3` gap and the 28px ellipsis — rather than the 170px it was given, and what it is for is the row that draws no actions at all: it stops the column collapsing and reflowing the six beside it |
| Group header | `<tr class="sm-group"><th colspan="7" scope="rowgroup">`, padding `var(--space-2) var(--space-8)` — 28px, pitch 29 — ground `--bg-surface`, bottom rule `--border`. It scrolls; it does not stick and does not collapse | left: `folder` codicon at `lg`, label `--text-base` / `--lh-base` semibold (which is what makes the 28 exact: 20 + 2 × `--space-2`), visible-row count `--text-xs` / `--lh-xs` at `--text-muted`, and the shipped `.chip-sm`: `Open tab` (transparent border, `--text-muted`) or `Tab closed` (`1px solid var(--border-strong)`, `--text-secondary`). Right: the path `--text-xs` / `--lh-xs`, ellipsis, `max-width: 40%`. It takes `.section-header`'s height without its uppercase |
| Status dot | the shipped `.dot` family, 8px | the kind comes from `statusVisual`; NO new shape is invented for a state a person cannot act on |
| Inline panel | `padding: var(--space-6); border: 1px solid var(--border-strong); border-radius: var(--r-md); background: var(--bg-raised); scroll-margin-top: calc(var(--sm-head-h) + var(--space-6))`. Its row's ground is `--bg-active` | title `--text-base` semibold, body `--text-sm` at `--lh-base`, `max-width: 80ch`, buttons right, Cancel first. An error line is `--error`, above the buttons. The scroll margin is what its 60px literal always was: the sticky heading's height plus a gap, so it follows the heading rather than being re-guessed |
| Batch panel | styled as an inline panel, margin `var(--space-5) var(--space-7)`, at the top of the scroller. Its target list: `max-height: calc(4 * 33px + var(--space-5))`, scrolling, ruled | four whole rows and a visible hint of the fifth. The 144px it replaces cut the fifth 33px row mid-row, which reads as the end of the list |
| Past row | flex, `min-height: var(--sm-row-h)`, `padding: var(--space-3) var(--space-8)` — one 40px row, and its pitch is **40**, because `* { box-sizing: border-box }` holds this row's hairline INSIDE its declared height where the grid's sits between two rows; identity (the name, then, inline beside it rather than stacked under it, the agent, the FOLDER the session ran in as the old panel drew it, and what Restore will do; the whole folder is the row's hover, and the machine is the name line's badge and is not repeated), a content-sized state slot reading `Removed <date>` with the exact time on hover, then Restore and the ellipsis | not a table: no headings, no checkboxes, no sort. ONE list in removal order, newest first, exactly as main answers it and as the old panel drew it. It groups under a project's header only when the project filter names one project, and the header then says the folder, so the small line leaves it out unless the session ran outside that project (the operator's ruling, 2026-09-19, and the reverify that put the folder back in place of the project's name). That whole secondary line is ONE field and it is the field that gives way, `flex: 0 100 auto` with the ellipsis on it, an order of magnitude above the name's shrink factor, and it truncates from the RIGHT rather than with `direction: rtl`, because bidi reordering moves a path's `/` and `.` runs. Turning the promise into the ↺ mark `.srow-saved` already draws, with the sentence in the hover and the panel, is the shape Phase 298 stopped short of: it takes `Starts fresh` off a resting row, which is a fourth change to what a person reads where the phase allows three, so it is its own entry. A machine-removed row keeps its tombstone on its own line and is the ONE row in the sheet above 40px |
| Toast strip | `.sm-toasts`, a `flex: 0 0 auto` column between the scroller and the footer, `align-items: flex-end`, `gap: var(--space-4)`, `padding: var(--space-4) var(--space-8)`; `:empty` it is `display: none` with no padding, so it takes no height | while the sheet is open the toast stack docks INSIDE it and covers nothing. A fixed toast at `--z-toast` 700 sits over `--z-modal` 500 and took the End column's clicks on the lowest rows, which is the one thing that was still stacked on the sheet. `pointer-events: none` is refused: a sticky toast's only exit is its own × and its action button, so it would be undismissable for the life of the window |
| State block | centred, `max-width: 540px`, padding `70px var(--space-8)`, a 24px codicon, the heading at `--text-lg` / `--lh-lg` and the body at `--text-base` / `--lh-base`, both classed rather than bare elements. Loading: five skeleton rows at `var(--sm-row-h)` in `--bg-raised`, with `.sm-loading`'s gap at 0 so the loading pitch is the row's own | replaces the grid, headings included; title bar, toolbar and footer stay. 24 is the app's ceiling and one of the four sizes `<Codicon>`'s own doc sanctions off the 12/14/16 scale; 28 was larger than anything the app draws. The skeleton was derived from the old two-line row, so against a 40px row it was 30px per row too tall and the list visibly resettled when the real rows landed |
| Footer | `padding: var(--space-3) var(--space-8); border-top: 1px solid var(--border); font-size: var(--text-2xs); line-height: var(--lh-2xs); color: var(--text-muted)` — 29px | kept at the 10px step deliberately: a footer is a footnote, which is what that step is for. It is the ONE user of `--text-2xs` in the sheet outside a chip |
| Buttons | End: `.btn.btn-secondary.btn-sm`, hover `--error-wash` ground, `--error` border and text. Restore: `.btn.btn-secondary.btn-sm`. Confirm: `.btn.btn-destructive`. Primary inline: `.btn.btn-primary` | |
| Focus | `box-shadow: var(--focus-ring)` on `:focus-visible` | |
| Narrow | the grid scroller scrolls sideways. Below 1100px the cell padding drops to `var(--space-3) var(--space-4)` and the name to 135px | only the HORIZONTAL term moves, so the narrow row can never be TALLER than the wide one. The 1100 stays a literal because a custom property is not readable in a media query |
| Icon buttons and the row's verbs | the refresh, the close and the ellipsis all carry the shipped `icon-btn` class and keep only their placement and their 28px size; their hover is `--bg-raised`, the value twenty `.icon-btn` sites use — except on a row, where the row's own hover has already taken `--bg-raised`, so the row's button goes to `--bg-active` | every icon in the sheet is 12, 14, 16 or 24, and no size appears here that appears nowhere else in the app. The row's action buttons are PERMANENTLY drawn and never revealed on hover, because a management sheet's verbs are the reason the sheet exists |
| Interaction | rows hover to `--bg-raised` and select to `--bg-active`, each `transition: background var(--dur-fast) var(--ease-out)`; the icon buttons, the tabs and the sort headings ease background and colour; `.btn.sm-end` names `background, border-color, color`, because its hover moves three. The keyboard cursor inside a checked block is `inset 0 0 0 1px var(--border-strong)`, or `--accent` when that row is also checked, gated on `.sm-scroll:focus-within` | `--dur-fast` is the step `.scm-row` and `.ptab` use, and a modal table row is closest in kind to those. The keyboard cursor is adopted verbatim from `scm.css`, which exists for this exact problem. The checked row keeps `border-bottom-color: var(--border-active)` and never `.srow.selected`'s inset outline: a full-width table row on `--bg-surface` is not `.srow` on `--bg-sidebar`, and `project-rail.css` already forbids copying that outline onto a full-bleed row. There is no `:active` state and no row radius |

- **Seven columns**: select-all, Session, State, Created, Messages, Last message, Actions. Each heading but the first and last is a sort button (`arrow-both` unsorted, `arrow-up`, `arrow-down`; the `th` carries `aria-sort`). Sorting reorders rows INSIDE each group and groups never reorder; a missing value sorts last in both directions; nothing is sorted on first open.
- **A group is one workspace target**, the folder AND the machine it is on, never a folder's basename. A closed tab filters nothing out. Groups with no visible row are left out.
- **State** is the shipped dot and the shipped label with its first letter raised: Working, Needs input (in `--status-attention`), Idle, Ended, Failed, Killed, Saved, Not running, Unreachable. **Created** is the day (`Today`, `Yesterday`, `Sep 16`) over a two-unit age (`5h 14m old`), and a session whose creation time is unknown draws a dash. **Messages** and **Last message** never draw a missing value as zero: a shell and a session on another machine draw a dash with the reason below it, partial history is marked `+`, and the cell says which clock it drew.
- **One visible button per row**: `End session…` on a live row, `Restore` on an ended or past one, disabled with its reason as the title where the shipped policy refuses. An unreachable row offers reads only. The ellipsis opens a NATIVE menu through `ui:popupMenu`: `Session details`, `Go to session`, a separator, then exactly what the session's menu offers everywhere else. No menu is DOM-drawn.
- **Inline, never stacked.** Details, rename, saved output, every confirmation and every error open as ONE panel under their row; no modal opens above the sheet. Initial focus goes to the panel's close button or its text field, never to a destructive button.
- **Batch End.** Row checkboxes and a header checkbox that selects every row the filters leave; clicking it with anything already checked clears. One confirmation names each running session it will end, counts the skipped ones with the reason, and then reports row by row. The list it named may shrink before the press and never grows.
- **Keyboard and focus.** Open puts focus in the search field. Tab wraps inside the sheet, and focus never rests outside it while it is the top layer. Left, Right, Home and End move between the two tabs. Escape takes one layer at a time: a non-empty search, a running batch (asked to stop), a batch panel, an open panel, then the sheet. F2 opens the focused row's rename and never renames a session behind the sheet. While the sheet is open ⌘T, ⇧⌘U, ⇧⌘↩, the split arrows, an agent hotkey, New Project…, Open Remote Project…, Clone Repository… and the Session menu's Resume and Rename rows act on the sheet or not at all. ⌘J, ⌘/, End Session… and Close Project… close the sheet first and then open over the app, so nothing is ever stacked on the sheet. Both doors open the sheet over the Catch Me Up page and the New Session sheet, which stay under it. A double click presses a control once. Close gives the keyboard back to where it was, and a keyboard that falls out of the sheet comes back to where it was in it. A restore from the Past Sessions tab closes the sheet and lands in the session, as the old panel did; the Managed tab stays open after a restore.
