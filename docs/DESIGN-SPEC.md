...
## S15 — Session manager (Session → Manage Sessions…) — Phase 293
...
**Rules that bind every line of its CSS and copy.** Tokens only, and no token was added for it; the four geometry numbers (52, 47, 46, 1180) are custom properties on `.session-sheet`. `--text-muted` only on `--bg-surface`; on `--bg-raised` and `--bg-active` grounds text is `--text-secondary` (S12 item 4). A hairline drawn ON `--bg-active` is `--border-active`; between regions it is `--border`. Class prefix `sm-`; the root is `.modal.session-sheet`, and an override of a shared modal class carries that second class so it wins on specificity and never on bundle order. No multiplexer vocabulary (S12 item 6). The resting face carries labels; explanation lives in a heading's hover title or inside the Details expansion. Reduced motion: the sheet uses `.modal`'s own entrance and nothing else animates.
...
| Title | `Sessions`, `--text-md` at `--weight-semibold` | `.modal-title` is 20px and too tall for a 52px bar |
| Tabs | `Managed` (codicon `terminal`) and `Past Sessions` (codicon `history`), each with a count chip: `--text-xs`, `--r-sm`, padding `2px 5px`, min-width 19px, ground `--bg-raised`. Selected: `--text-primary` and a 2px `--accent` underline at `bottom: -1px` | the counts are whole-tab totals and filters never move them |
...
| Toolbar, filters mode | search `flex: 1; min-width: 180px`; project select 170px; tab select 136px; state select 114px (Managed only). Selects `--field-h`, `--text-xs` | the four controls combine with AND |
...
| Sticky headings | `thead th`: `position: sticky; top: 0; z-index: 1; background: var(--bg-raised); padding: var(--space-5); font-size: var(--text-2xs); font-weight: var(--weight-medium); color: var(--text-secondary); white-space: nowrap`. The rule under them is `box-shadow: inset 0 -1px 0 var(--border-strong)` | a border on a sticky `th` under `border-collapse: collapse` drops out in Chromium |
| Data cell | `padding: var(--space-6) var(--space-5); border-bottom: 1px solid var(--border); vertical-align: middle; white-space: nowrap`. A cell's second line is `small`: block, `--text-2xs`, line-height 1.5, `margin-top: var(--space-3)`, `--text-secondary` | row hover `--bg-raised` |
...
| Session column | `min-width: 155px`; the name `max-width: 175px`, ellipsis | agent mark 19px, name, machine badge for another machine only, agent label below |
| Actions column | `min-width: 170px; padding-right: var(--space-7)`; `.sm-row-actions { display: flex; justify-content: flex-end; gap: var(--space-3) }` | ONE visible button, then the ellipsis |
| Group header | `<tr class="sm-group"><th colspan="7" scope="rowgroup">`, padding `var(--space-6) var(--space-8) var(--space-2)`, ground `--bg-surface`, bottom rule `--border`. It scrolls; it does not stick and does not collapse | left: `folder` codicon, label `--text-sm` semibold, visible-row count `--text-muted`, and a chip `--text-2xs`, padding `1px 6px`, `--r-sm`: `Open tab` (transparent border, `--text-muted`) or `Tab closed` (`1px solid var(--border-strong)`, `--text-secondary`). Right: the path `--text-xs`, ellipsis, `max-width: 40%` |
...
| Inline panel | `padding: var(--space-6); border: 1px solid var(--border-strong); border-radius: var(--r-md); background: var(--bg-raised); scroll-margin-top: 60px`. Its row's ground is `--bg-active` | title `--text-base` semibold, body `--text-sm` at `--lh-base`, `max-width: 80ch`, buttons right, Cancel first. An error line is `--error`, above the buttons |
| Batch panel | styled as an inline panel, margin `var(--space-5) var(--space-7)`, at the top of the scroller. Its target list: `max-height: 144px`, scrolling, ruled | |
| Past row | flex, `min-height: 64px`; identity (the name, then a small line: the agent, the FOLDER the session ran in as the old panel drew it, and what Restore will do; the whole folder is the row's hover, and the machine is the name line's badge and is not repeated), a 145px state slot reading `Removed <date>` with the exact time on hover, then Restore and the ellipsis | not a table: no headings, no checkboxes, no sort. ONE list in removal order, newest first, exactly as main answers it and as the old panel drew it. It groups under a project's header only when the project filter names one project, and the header then says the folder, so the small line leaves it out unless the session ran outside that project (the operator's ruling, 2026-09-19, and the reverify that put the folder back in place of the project's name) |
| State block | centred, `max-width: 540px`, padding `70px var(--space-8)`, a 28px codicon, the heading at `--text-md`. Loading: five 62px skeleton rows in `--bg-raised` | replaces the grid, headings included; title bar, toolbar and footer stay |
...
- **Seven columns**: select-all, Session, State, Created, Messages, Last message, Actions. Each heading but the first and last is a sort button (`chevron-expand` unsorted, `arrow-up`, `arrow-down`; the `th` carries `aria-sort`). Sorting reorders rows INSIDE each group and groups never reorder; a missing value sorts last in both directions; nothing is sorted on first open.
