...
**Touching the file walk?** Add `npm run conformance:filehistory` (about 3 s, launches no Electron, starts no tmux server, spawns no agent, makes no request, reads nothing under the person's home) for any commit under `src/main/git/service.ts`'s `walk`, `src/main/git/parse.ts`'s `readNameStatusChunk` or `src/main/git/graph-parse.ts`. It builds a ten commit fixture holding a copy, a rename to a name with `*` and `[` in it, an in place rewrite, a delete and a merge, runs the SHIPPING walk over it and pins every row's status, path and old path, and then runs the same probe over three ablated copies of the shipping modules, the newline strip taken out of the chunk reader, `--follow` taken out of the argv and `--topo-order` put back under follow, each of which must go red. The third one is the finding this phase was built on: on git 2.50.1 `--topo-order` changes what `--follow` returns, 607 rows against 609 over `builtin/log.c` in git's own repository and 8 against 9 on the fixture, the missing rows being ordinary edits on a side branch merged after the rename, so a followed walk drops the flag. `npm run probe:p198` (a few minutes, one Electron on a scratch profile, spawns no agent, spends no token) is the app run over the gmux copy and is not in the commit battery.
