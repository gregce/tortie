...
| `src/renderer/editor/markdown/window-scan.ts`, `chunk-parse.ts`, `pipeline.ts`, `large-prose.ts`, the windowed half of `markdown-impl.tsx`, `build/p254/twins.mjs` | `conformance:preview` | ~60 s, no Electron | The windowed preview draws the page the old renderer drew: the chunked render equals the whole render over this repository's markdown and both twins, every file that differs from the old react-markdown render sits in a pinned class, the parity shims and the `www.` residual are pinned, the sanitizer still runs before Shiki, the window caps, the re-derived deferral guard and the chunk memo are pinned, a chunk mounts as a fragment so `conformance:wideblocks`' direct-child selectors still match — and 23 ablations, one clause each, go red on the rule that owns them |
