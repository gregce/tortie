...
**Touching a script under `build/` that runs ssh?** Add `npm run gate:knownhosts` (about 0.5 s, spawns nothing, starts no ssh and no sshd, opens no socket, reads nothing under the person's home, all four measured by running it with child_process, net, tls, dgram and fs wrapped rather than by reading its source) for any commit under `build/`. It runs inside `npm run build` too, so nothing that builds can skip it. It asserts that no file under `build/` except `build/ssh-run.mjs` hands ssh, scp, sftp or ssh-keyscan to a spawn, that no spawn hands `ssh-keygen` its `-R` or `-F` flag, that no file names one of the four on a shell command line, that all 19 scripts that run one still reach the helper, and that inside the helper the `-o UserKnownHostsFile=` pair is emitted from exactly one place, refuses an empty value, has no default so forgetting it throws, is prepended so nothing later in an argv can win, and puts Tortie's own record file first. It proves the last four by RUNNING the helper as well as reading it, and it proves its own scanner on six fixtures it writes itself, five of which must make it fail. The product side of this was already gated by `npm run conformance:machines`; the harness was not, and eighteen scripts each carried the option by hand, which is eighteen facts about today rather than a guarantee. Run against the tree before Phase 193, this gate finds 38 unrouted spawns and one `ssh -V` on a `/bin/sh -c` line.
