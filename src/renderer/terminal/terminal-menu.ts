...
export function terminalMenuItems(
  ...
  if (canCapture) {
    items.push('sep', {
      // A CHOSEN mark, and EVERY capture row wears it, because each row is a
      // different extent of the same photograph and all of them put an image
      // on the clipboard. Four rows are drawn for a session on this Mac and
      // two for one on another machine; this comment said three, which Phase
      // 205 corrected along with the measurement behind the mark. The
      // reasoning and that measurement are in src/shared/menu-codicons.ts.
