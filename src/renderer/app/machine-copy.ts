...
/**
 * Why History is not on screen.
 *
 * Said once, under the Changes group, rather than an empty section. History
 * needs a read Tortie does not make on another machine, and an empty section
 * would read as a repository with no history.
 *
 * PHASE 105 TOOK RUNS OUT OF THIS SENTENCE, and PHASE 106 TOOK BRANCHES OUT.
 * It named three sections at Phase 90.3 and each round that shipped one of them
 * made another clause false. History is the one that is left, and it is still a
 * read Tortie does not make over there.
 *
...
export const REMOTE_SCM_SECTIONS_ABSENT =
  'Tortie shows the changed files, the branch and the runs for a folder on ' +
  'another machine. It does not show history there.';
