/**
 * Appearance persistence (Phase 62) — sanitize plus a store round trip.
 *
 * The two fields are preferences with no danger semantics. They never touch
 ...
 * - an old settings file without either key loads as the defaults;
 * - the file that lands on disk carries both fields, so the next load does
 *   not depend on defaults being re-derived.
 */
  ...
  it('every valid value survives sanitize', async () => {
    ...
    for (const highlightScheme of ['blue', 'teal', 'purple', 'slate']) {
      for (const contrastLevel of ['normal', 'raised', 'high']) {
        const out = store.sanitizeSettings({ highlightScheme, contrastLevel });
...
describe('the store round trip', () => {
  it('a valid value survives a write and a fresh load', async () => {
    ...
    first.updateSettings({ highlightScheme: 'purple', contrastLevel: 'raised' });

    // What landed on disk carries both fields explicitly.
  ...
  it('an old settings file without either key loads as the defaults', async () => {
    writeFileSync(
